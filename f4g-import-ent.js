/* =========================================================================
   FARM4GLASS — ENTREPRENEURSHIP IMPORTER (self-contained)
   -------------------------------------------------------------------------
   This file carries its own copy of the importer with the course hardcoded to
   "entrepreneurship-cluster". It cannot write to any other course. There is
   no bank swap and no options object to mistype.

   HOW TO USE
   1. Upload this file to the repo next to index.html.
   2. In index.html, REPLACE the f4g-questions-ent.js line with:

        <script type="module" src="f4g-import-ent.js"></script>

      (Delete the old f4g-questions-ent.js line. This file contains the
      questions AND the importer, so you don't need both.)

   3. Reload, sign in as admin, open the console, and run:

        f4gPreviewEnt()      // shows what would happen — writes NOTHING
        f4gImportEnt()       // actually writes, to entrepreneurship only

   4. When you're done, delete the script tag. Keep the file in the repo.

   SAFETY
   - TARGET_COURSE below is the only course this file will ever touch.
   - f4gPreviewEnt() never writes.
   - f4gImportEnt() appends only; it never overwrites or deletes a question,
     and it skips anything already present. Running it twice is harmless.
   ========================================================================= */
...
/* The ONLY course this file can write to. If your entrepreneurship cluster
   course has a different ID in Admin > Courses, change it here — this is the
   single place it appears. */
const TARGET_COURSE = "entrepreneurship-cluster";
...
/* ---- deterministic option shuffle --------------------------------------
   ...
   question's options and moves the answer index with them. Seeded from the
   question text, so the same question always shuffles the same way. */
...
async function run(apply) {
  ...
  const total = BANK.reduce((n, e) => n + e.questions.length, 0);
  console.log(
    `%cTARGET COURSE: ${TARGET_COURSE}`,
    "font-weight:bold;font-size:14px;color:#167db5"
  );
  console.log(
    `Bank: ${BANK.length} instructional areas, ${total} questions.\n` +
    (apply ? "MODE: APPLY — writing now." : "MODE: PREVIEW — nothing will be written.")
  );

  const ref = doc(db, "courses", TARGET_COURSE);
  const snap = await getDoc(ref);
  if (!snap.exists()) {
    console.error(
      `No course document "${TARGET_COURSE}" in Firestore. Check the ID in ` +
      `Admin > Courses and update TARGET_COURSE at the top of f4g-import-ent.js.`
    );
    return;
  }
  ...
  const report = [];
  let added = 0, skipped = 0, unitsCreated = 0;
  ...
  for (const entry of BANK) {
    ...
    if (!unit) {
      const key = `${TARGET_COURSE}-u${Date.now()}${newUnitNumber}`;
      unit = { key, title: `Unit ${newUnitNumber}: ${entry.ia}` };
      newUnitNumber++;
      createdUnit = true;
      unitsCreated++;
    }
    ...
    report.push({
      ...
      Added: entryAdded,
      Skipped: entrySkipped,
      Note: createdUnit ? "NEW UNIT created" : ""
    });
  }

  console.table(report);
  console.log(`${added} question(s) · ${skipped} already present · ${unitsCreated} unit(s) created`);

  if (!apply) {
    console.log(
      "PREVIEW ONLY — nothing was written.\n" +
      "When it looks right, run:  f4gImportEnt()"
    );
    return report;
  }
  ...
  try {
    ...
    console.log(
      `%cWritten to ${TARGET_COURSE}. Reload the page and open it to check.`,
...
window.f4gPreviewEnt = () => run(false);
window.f4gImportEnt = () => run(true);

console.log(
  `%cEntrepreneurship importer ready — locked to "${TARGET_COURSE}".`,
  "font-weight:bold;color:#167db5"
);
console.log("  f4gPreviewEnt()   preview, writes nothing");
console.log("  f4gImportEnt()    write to entrepreneurship");
